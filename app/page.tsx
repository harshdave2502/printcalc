...
// ─── GSM CONVERTER HELPER ─────────────────────────────────────────────
function gsmInfo(gsm:number):string{
  if(!gsm)return '';
  // Text paper (thin: < 170 GSM): GSM ÷ 1.48 = lb Text
  // Cover paper (thick: ≥ 170 GSM): GSM ÷ 2.71 = lb Cover
  // PT: approximate from common industry chart
  const ptMap:Record<number,number>={60:4,70:4,80:5,90:5,100:6,115:7,120:7,130:8,150:9,157:9,
    170:10,200:12,230:13,250:14,300:16,350:18,400:20};
  const pt=ptMap[gsm]||Math.round(gsm/20);
  if(gsm<170){
    const lb=Math.round(gsm/1.48);
    return `= ${lb} lb Text · ~${pt}pt`;
  } else {
    const lb=Math.round(gsm/2.71);
    return `= ${lb} lb Cover · ~${pt}pt`;
  }
}

// ─── SIZE SELECTOR with unit converter ───────────────────────────────
function SizeSelect({size,setSize,cW,setCW,cH,setCH}:any){
  ...
  // Convert display value to inches for calculation
  const toIn=(v:string)=>{
    const n=parseFloat(v)||0;
    if(unit==='mm')return(n/25.4).toFixed(3);
    if(unit==='cm')return(n/2.54).toFixed(3);
    return v;
  };
  // Convert inches to display unit
  const fromIn=(v:string)=>{
    const n=parseFloat(v)||0;
    if(unit==='mm')return(n*25.4).toFixed(1);
    if(unit==='cm')return(n*2.54).toFixed(2);
    return v;
  };

  const handleW=(v:string)=>setCW(toIn(v));
  const handleH=(v:string)=>setCH(toIn(v));
  const dispW=cW?fromIn(cW):'';
  const dispH=cH?fromIn(cH):'';
  const placeholder=unit==='mm'?'e.g. 210':unit==='cm'?'e.g. 21':'e.g. 8.3';
      ...
      {size.id==='custom'&&(
        <div style={{marginTop:8}}>
          <div style={{display:'flex',gap:6,marginBottom:8}}>
            <span style={{fontSize:11,color:'#888',alignSelf:'center'}}>Unit:</span>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            <input type="number" placeholder={`Width (${unit}) ${placeholder}`} value={dispW} onChange={e=>handleW(e.target.value)} style={NIS}/>
            <input type="number" placeholder={`Height (${unit})`} value={dispH} onChange={e=>handleH(e.target.value)} style={NIS}/>
          </div>
      ...
      {jobType==='single'&&(
        <>
          ...
          <Sec title="Printing">
            ...
            <div><div style={LBL}>Sides</div><div style={TW}><button style={TB(sides==='single')} onClick={()=>setSides('single')}>Single side</button><button style={TB(sides==='double')} onClick={()=>setSides('double')}>Both Sides</button></div></div>
          </Sec>
