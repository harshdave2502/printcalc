    ...
    try {
      ...
      if(authData.user){
        ...
        const {error:profileError} = await supabase.from('subscribers').insert({
          ...
          markup_percent:25,tax_percent:18,
          plan:'free',
          status:'pending',
  ...
  if(success){return(
    <>
      ...
      <main style={{minHeight:'100vh',background:'#F7F6F3',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
        <div style={{background:'#fff',borderRadius:16,padding:40,maxWidth:460,width:'100%',textAlign:'center',border:'1px solid #EBEBEB'}}>
          <div style={{fontSize:48,marginBottom:16}}>⏳</div>
          <h2 style={{fontSize:22,fontWeight:600,color:'#1A1A1A',marginBottom:8}}>Application submitted!</h2>
          <p style={{fontSize:15,color:'#555',marginBottom:8}}>{businessName} — we've received your signup.</p>
          <div style={{background:'#FFFBEB',border:'1px solid #FDE68A',borderRadius:10,padding:16,marginBottom:20,textAlign:'left'}}>
            <p style={{fontSize:13,fontWeight:600,color:'#78350F',marginBottom:6}}>What happens next:</p>
            <p style={{fontSize:13,color:'#92400E',lineHeight:1.7}}>
              1. Our team reviews your application (usually within 24 hours)<br/>
              2. You'll receive an approval email at <strong>{email}</strong><br/>
              3. Once approved, you can login and start using PrintCalc
            </p>
          </div>
          <p style={{fontSize:12,color:'#AAA'}}>Questions? Email us at support@printcalc.app</p>
        </div>
      </main>
    </>
  );}
